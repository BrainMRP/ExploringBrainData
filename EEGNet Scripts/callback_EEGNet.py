...
class PrintingCallback(Callback):
    def __init__(self, number_epochs, batch_size, EEGNet_model_object, using_gpu):
        self.number_epochs = number_epochs
        self.batch_size = batch_size
        self.EEGNet_model_object = EEGNet_model_object
        self.create_main_experiment_folder()
        self.create_EEGNet_folder()
        self.experiment_number = self.get_experiment_number()
        self.create_experiment_folder(self.experiment_number)
        self.using_gpu = using_gpu
        #self.checkpoint_callback = self.model_checkpoint()
    ...
    def on_train_begin(self, logs=None):
        ...
        print()
        if(self.using_gpu):
            print("-"*7 +" Beginning of Experiment {} of the EEGNet model using GPU ".format(self.experiment_number) + "-"*7)
        else:
            print("-"*7 +" Beginning of Experiment {} of the EEGNet model without using GPU".format(self.experiment_number) + "-"*7)            
        print()
        print()
        # self.create_experiment_folder(self.experiment_number)
        self.create_summary_file(self.experiment_number)
        self.append_to_summary_file(self.EEGNet_model_object, self.experiment_number)
    ...
    def model_checkpoint(self):
        exp_path = "Experiments/EEGNet/Experiment" + str(self.experiment_number)
    ...
    def create_EEGNet_folder(self):
        path_EEGNet = "Experiments/EEGNet"
        if(not os.path.isdir(path_EEGNet)):
            try:
                os.mkdir(path_EEGNet)
            except Exception as e:
                print ("Creation of the main EEGNet experiment directory failed")
                print("Exception error: ",str(e))     
    ...
    def create_experiment_folder(self,experiment_number):
        try:
            path_new_experiment = "Experiments/EEGNet/Experiment" + str(experiment_number)
    ...
    def create_summary_file(self,experiment_number):
        filename = "Experiments/EEGNet/Experiment"+str(experiment_number)+"/Experiment_Summary"+str(experiment_number)+".txt"
    ...
    def append_to_summary_file(self, model_object, experiment_number):
        return
        """
        filename = "Experiments/EEGNet/Experiment"+str(experiment_number)+"/summary_model"+str(experiment_number)+".txt"
    ...
    def create_info_epochs_file(self,experiment_number):
        filename = "Experiments/EEGNet/Experiment"+str(experiment_number)+"/info_epochs_model"+str(experiment_number)+".txt"
        with open(filename, "w") as file:
            file.write("")
    ...
    def create_info_epochs_prediction(self, experiment_number):
        filename = "Experiments/EEGNet/Experiment"+str(experiment_number)+"/info_prediction_model"+str(experiment_number)+".txt"
        with open(filename, "w") as file:
    ...
    def plot_epochs_info(self,experiment_number):
        ...
        ax2.set(ylabel="Loss")
        output_filename = "Experiments/EEGNet/Experiment"+str(experiment_number)+"/plot_model"+str(experiment_number)+".jpg"
